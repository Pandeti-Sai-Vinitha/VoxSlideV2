...
from config import EXTRACTED_IMAGES_DIR, get_pdf_workspace, parse_versioned_basename
...
def _deduplicate_images_across_slides(slides: List[Dict], logger) -> List[Dict]:
    ...
    logger.info(
        f"  ✅ Image distribution: {len(used_images)} unique images in {len([s for s in slides if s.get('image_index') is not None])} slides")
...
def _distribute_unused_images(slides: List[Dict], available_images: List[str], logger) -> List[Dict]:
    ...
    logger.info(
        f"  Distributing {len(unused_images)} unused images to {len(slides_without_images)} slides without images")
...
def _generate_slides_hierarchical(input_file, pdf_basename, paths, logger, token_usage, persona, num_slides,
    ...
    try:
        ...
        if ext == 'docx':
            ...
            if version_dir:
                images_folder = EXTRACTED_IMAGES_DIR / base_name / version_dir
            else:
                images_folder = EXTRACTED_IMAGES_DIR / base_name / "v1"
            logger.info(f"Using DOCX extracted images folder: {images_folder}")
        blocks, hierarchy, image_paths = extract_with_hierarchy(input_file, ext, images_folder=str(
            images_folder) if images_folder else None)
        ...
        global_tokens = count_tokens_tiktoken(
            global_summary.summary if hasattr(global_summary, 'summary') else str(global_summary))
        ...
        def generate_slide(plan):
            ...
            # Inject persona-specific instructions and any extra user instructions
        ...
        logger.info("PHASE 12: VALIDATING AND FIXING SLIDE CONTENT")
        slides_json_data = {"slides": slides}
        if voice_name:
            slides_json_data["voice_name"] = voice_name
            logger.info(f"Saving voice_name to slides.json: {voice_name}")
        if template_name:
            slides_json_data["template_name"] = template_name
            logger.info(f"Saving template_name to slides.json: {template_name}")
        
        with open(paths["slides_json"], "w", encoding="utf-8") as f:
            json.dump(slides_json_data, f, indent=2)

        validation_report = validate_and_fix_slides_json(
            slides_json_path=str(paths["slides_json"]),
            auto_fix=True,
            verbose=False
        )
        logger.info(f"  ✓ Slides validated and fixed: {validation_report.get('fixed_slides', 0)} slides updated")

        logger.info("PHASE 13: RELOADING FIXED SLIDES")
        with open(paths["slides_json"], "r", encoding="utf-8") as f:
            slides_data_reloaded = json.load(f)
            slides = slides_data_reloaded.get("slides", [])
        
        # ✅ REAPPLY VOICE_NAME AFTER VALIDATION (validation may have overwritten it)
        if voice_name and slides_data_reloaded.get("voice_name") != voice_name:
            slides_data_reloaded["voice_name"] = voice_name
            with open(paths["slides_json"], "w", encoding="utf-8") as f:
                json.dump(slides_data_reloaded, f, indent=2)
            logger.info(f"✅ Reapplied voice_name to slides.json after validation: {voice_name}")
    ...
    try:
        ...
        for idx, slide in enumerate(slides, 1):
            ...
            if audio_path.exists():
                logger.info(f"  [{idx}/{len(slides)}] Audio already exists, skipping: {audio_path}")
                continue
            voiceover_text = slide.get('voiceover') or slide.get('audio_script') or slide.get('audioScript') or ''
            if not voiceover_text:
                logger.warning(f"  [{idx}/{len(slides)}] No voiceover text found for slide {idx}, skipping")
                continue
            logger.info(f"  [{idx}/{len(slides)}] Creating voiceover: {audio_path} (voice: {voice_name or 'default'})")
        ...
        logger.info(f"  ✓ All {len(slides)} voiceovers created or skipped")
        # Create PPT
        logger.info("-" * 60)
        ...
        for candidate in candidates:
            if os_module.path.exists(candidate):
                ...
                logger.info(f"  ✓ Found extracted images folder: {images_folder}")
                break

        if not images_folder:
            logger.info(f"  ⚠️  No extracted images found (tried: {', '.join(candidates)})")
        ...
        if len(slide_images) != len(slides):
            logger.warning(
                f"  ⚠️  Mismatch: slides.json has {len(slides)} slides but PPT exported {len(slide_images)} images")
